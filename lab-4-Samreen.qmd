...
five <- read.table("46092h2025.txt")
```
...
write_csv(buoy, file = "data-clean/bouy_update.csv") 
bouy_update <- read.csv("data-clean/bouy_update.csv")
```
...
#| label: parse-double
#| eval: false
buoy_final <- bouy_update |> 
  mutate(across(.cols = everything(), ~parse_double(.x)))
write_csv(buoy, file = "data-clean/bouy_final.csv")
```
...
Now that are data is "analytic ready", go ahead and read in your clean data and use it to try and recreate the visualization in the instructions. Do your best to match the following visualization as close as possible. (Hint: when specifying color, you may need to use `factor()` to get R to treat a continuous value as discrete).  
...
#| message: false
 ...
 ggplot(buoy_final, aes(x = MM, y = ATMP)) +
   geom_jitter(aes(color = MM), alpha = 0.1, width = 0.2) +
   geom_smooth(color = "black") +

  labs(
    title = "Monterey Bay Canyon Air Temperature in 2025",
    x = "Month in 2025",
    y = "Air Temperature in Celsius") +
  theme_minimal()
...
>APOD Astronomy Picture of the day are a daily astronomy picture from NASA's website that is provided with a description. THe data is a collection of what picture and what description was posted each day. 
We can ask the following research questions:
1) What are the most frequently posted-about types of space features and is there a public interest in a space feature that is associated with this bout of information?
2) Does public interest in a certain space feature influence what the daily archive posts?
